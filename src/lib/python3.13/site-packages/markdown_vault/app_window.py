...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _show_save_dialog(self, dirty_paths: list[str], on_confirm=None) -> None:
        ...
        logger.info("save-dialog: creating dialog for %d tab(s)", n)
        try:
            dialog = Adw.AlertDialog(
                heading="Unsaved Changes",
                body=f"{n} tab{'s' if n > 1 else ''} have unsaved changes:\n\n{body_lines}",
            )
            ...
            dialog.set_close_response("cancel")
            logger.info("save-dialog: Adw.AlertDialog created successfully")
            ...
            logger.error("save-dialog: failed to create dialog", exc_info=True)
            raise
    ...
    def _on_save_dialog_response(self, response: str, dirty_paths: list[str], on_confirm) -> None:
        ...
        if response == "cancel":
            ...
            return
        if response == "save":
            self._save_dirty_tabs(dirty_paths)
    ...
    def _save_dirty_tabs(self, paths: list[str]) -> None:
    ...
    # Matches a Markdown checkbox on a list line (0-based group 3 = state).
    _CHECKBOX_RE = re.compile(r'^(\s*)([-*+]|\d+\.)\s+\[([ x])\]')

    def _on_preview_checkbox_toggled(self, _preview, index: int, checked: bool) -> None:
        """Handle checkbox toggle — find the Nth checkbox in the editor and flip it."""
        logger.debug("Checkbox toggled: index=%s checked=%s", index, checked)
        tab = self._tab_bar.get_current_tab()
        ...
        # Find the Nth checkbox line in the current editor text.
        checkbox_index = 0
        target_line = -1
        for i, line_text in enumerate(lines):
            if self._CHECKBOX_RE.match(line_text):
                if checkbox_index == index:
                    target_line = i
                    break
                checkbox_index += 1

        if target_line < 0:
            logger.debug("Checkbox index %s not found in editor (found %s)", index, checkbox_index)
            return
        ...
        new_line = (
            original_line[:match.start(3)]
            + new_state
            + original_line[match.end(3):]
        )

        # Replace only the affected line in the buffer (preserves cursor/undo).
        buffer = tab.editor._buffer
        _ok, line_start = buffer.get_iter_at_line(target_line)
        _ok, line_end = buffer.get_iter_at_line(target_line)
        ...
        logger.debug("Checkbox toggled on line %s: %s -> %s", target_line, old_state, new_state)
