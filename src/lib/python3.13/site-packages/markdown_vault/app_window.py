...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _on_debug_toggled(self, toggle: Gtk.ToggleButton) -> None:
        ...
        if toggle.get_active():
            ...
            self._dump_file_index()
        else:
            toggle.remove_css_class("suggested-action")

    def _dump_file_index(self) -> None:
        """Write the file index to a JSON file if debug mode is active."""
        if not self._debug_toggle.get_active():
            return
        if self._settings.get("loglevel", "info") != "debug":
            return
        dump_path = config.STATE_DIR / "debug-file-index.json"
        self._file_index.dump_to_file(dump_path)
