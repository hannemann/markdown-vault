...
class MainWindow(Adw.ApplicationWindow):
    ...
    def _on_save_dialog_response(self, response: str, dirty_paths: list[str], on_confirm) -> None:
        ...
        if response == "save":
            ...
            if failed:
                ...
                dialog = Adw.AlertDialog(
                    heading="Save Failed",
                    body=f"Could not save {len(failed)} tab(s):\n\n"
                         + "\n".join(f"– {Path(p).name}" for p in failed),
                )
                dialog.add_response("ok", "OK")

                def _on_error_dismissed(_dlg, _resp):
                    if self._close_window_pending:
                        self._close_window_pending = False
                        self._restart_autosave()

                dialog.connect("response", _on_error_dismissed)
                dialog.present(self)
                return
    ...
    def _restart_autosave(self) -> None:
        """Restart the autosave timer (after a cancelled close request)."""
        self._cancel_autosave()
        self._setup_autosave()
    ...
    def _on_monitor_file_deleted(self, vault_path: str, file_path: str) -> None:
        ...
        if not file_path.endswith(".md"):
            ...
            self._refresh_sidebar_backlinks()
            return
        # Remove wikilinks from other files BEFORE removing from index
    ...
    def _on_monitor_content_changed(self, vault_path: str, file_path: str) -> None:
        ...
        def _update():
            try:
                ...
                text = ""
            self._backlink_index.update_file(file_path, text)
            self._dump_debug(["backlink_index", "preview_html", "sidebar"])
    ...
    def _refresh_sidebar_backlinks(self) -> None:
        """Refresh the sidebar backlinks for the currently active file."""
        tab = self._tab_bar.get_current_tab()
        if tab:
            self._sidebar.update_for_file(tab.editor.file_path, tab.editor.get_text())
