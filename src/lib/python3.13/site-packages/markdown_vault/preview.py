...
class CheckboxLinePostprocessor(Postprocessor):
    """Add data-checkbox-index attributes to checkbox <input> elements.

    Runs after pymdownx.tasklist so the extension has already generated
    the <input> elements.  Each checkbox gets a sequential index that
    maps back to the Nth checkbox pattern in the source.
    """

    CHECKBOX_INPUT_RE = re.compile(r'<input\s+type="checkbox"[^>]*/?>')
    # Matches the closing > or /> of an input tag.
    ...
    def run(self, text):
        ...
        for match in self.CHECKBOX_INPUT_RE.finditer(text):
            tag = match.group(0)
            # Remove disabled attribute if present (safety net).
            tag = re.sub(r'\s+disabled\b', '', tag)
            # Inject data-checkbox-index before the closing > or />.
            tag = self.TAG_CLOSE_RE.sub(
                f' data-checkbox-index="{checkbox_count}"\\1', tag,
            )
...
class CheckboxExtension(Extension):
    """Extension that adds checkbox line tracking via postprocessor."""
...
class Preview(Gtk.ScrolledWindow):
        ...
        checkbox_script = WebKit.UserScript.new(
            ...
            (function() {
                document.body.addEventListener('click', function(e) {
                    var el = e.target;
                    if (el && el.type === 'checkbox' && el.dataset.checkboxIndex !== undefined) {
                        var index = parseInt(el.dataset.checkboxIndex);
                        var checked = el.checked;
                        if (window.webkit && window.webkit.messageHandlers
                            && window.webkit.messageHandlers['checkboxHandler']) {
                            window.webkit.messageHandlers['checkboxHandler']
                                .postMessage({ index: index, checked: checked });
                        }
                    }
                });
            })();
    ...
    def _on_checkbox_clicked(self, _content_manager, jsc_value) -> None:
        ...
        try:
            ...
            GLib.idle_add(self.emit, "checkbox-toggled", index, checked)
        except Exception:
