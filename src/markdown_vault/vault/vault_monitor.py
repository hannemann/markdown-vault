...
def _is_valid_md_file(file_path, other_file_path=None):
    """Check whether it is a valid .md file.

    - Extension must be .md
    - Filename must not start with . (.hidden.md → ignored)
    - Parent directories must not start with . (.git/file.md → ignored)

    Args:
        file_path: Path of the file (Gio.File or str)
        other_file_path: Second path for MOVE events (Gio.File or str)
    ...
    # Skip hidden files (.hidden.md)
    if name.startswith("."):
        return False

    # Skip hidden directories (.git/file.md)
...
def _is_valid_md_dir(dir_path):
    """Check whether a directory is valid for monitoring.

    Args:
        dir_path: Path of the directory

    Returns:
        True if the directory should be monitored
    """
    if dir_path is None:
        return False
...
class VaultMonitor:
    ...
    def __init__(self):
        ...
        # Gio.FileMonitorEvent Mapping — CHANGES_DONE_HINT triggers changed
        self._EVENT_MAP = {
