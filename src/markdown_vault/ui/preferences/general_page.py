...
class GeneralPageMixin:
    def _build_general_page(self) -> None:
        ...
        current_mode = config.get_setting(self._settings, "view.default_mode", "edit")
    ...
    def _on_autosave_changed(self, _row: Adw.SpinRow, _pspec) -> None:
        config.set_setting(self._settings, "autosave.interval",
                           int(self._autosave_row.get_adjustment().get_value()))
        self._persist()
    ...
    def _on_view_mode_changed(self, row: Adw.ComboRow, _pspec) -> None:
        ...
        if idx < len(modes):
            config.set_setting(self._settings, "view.default_mode", modes[idx])
            self._persist()
