...
class TestSidebarRefreshEvent(unittest.TestCase):
    ...
    def test_sidebar_refresh_match_created(self):
        ...
        sidebar.refresh(
            FileEvent("/vault", "/vault/foo.md", "created"),
        )
        sidebar._refresh_outline.assert_called_once()
        sidebar._refresh_backlinks.assert_called_once_with("/vault/foo.md")
        sidebar._refresh_details.assert_called_once()
    ...
    def test_sidebar_refresh_match_deleted(self):
        ...
        sidebar.refresh(
            FileEvent("/vault", "/vault/foo.md", "deleted"),
        )
        sidebar._refresh_backlinks.assert_called_once_with("/vault/foo.md")
    ...
    def test_sidebar_refresh_match_moved(self):
        ...
        sidebar.refresh(
            FileEvent("/vault", "/vault/new.md", "moved", "/vault/old.md"),
        )
        sidebar._refresh_backlinks.assert_called_once_with("/vault/new.md")

    def test_sidebar_refresh_match_content_changed(self):
        """Sidebar.refresh() with 'content_changed' should call update_text_only()."""
        from markdown_vault.sidebar import Sidebar
        ...
        sidebar.refresh(
            FileEvent("/vault", "/vault/foo.md", "content_changed"),
        )
